...
class FavouriteScreenState extends State<FavouriteScreen> {
  ...
  /*scrollListenerFavouriteRestaurant() {
    if (controllerFavouriteRestaurant.position.maxScrollExtent == controllerFavouriteRestaurant.offset) {
      if (context.read<FavouriteRestaurantCubit>().hasMoreData()) {
        context.read<FavouriteRestaurantCubit>().fetchMoreFavouriteRestaurantData(context.read<AuthCubit>().getId(), partnersKey, perPage);
      }
    }
  }*/

  /*scrollListenerFavouriteProduct() {
    if (controllerFavouriteProduct.position.maxScrollExtent == controllerFavouriteProduct.offset) {
      if (context.read<FavouriteProductCubit>().hasMoreData()) {
        context.read<FavouriteProductCubit>().fetchMoreFavouriteProductData(context.read<AuthCubit>().getId(), productsKey, perPage);
      }
    }
  }*/
        ...
        builder: (context, state) {
          ...
          return SizedBox(
              ...
              child: GridView.count(
                  ...
                  controller: controllerFavouriteProduct,
    ...
    return BlocBuilder<FavoriteRestaurantsCubit, FavoriteRestaurantsState>(
        ...
        builder: (context, state) {
          ...
          return ListView.builder(
              shrinkWrap: true,
              padding: EdgeInsets.zero,
              controller: controllerFavouriteRestaurant,
              physics: const BouncingScrollPhysics(),
  ...
  Widget favourite() {
    return Container(
      margin: EdgeInsets.only(left: width! / 7.0, right: width! / 7.0),
      child: SingleChildScrollView(
        child: Column(mainAxisAlignment: MainAxisAlignment.start, crossAxisAlignment: CrossAxisAlignment.center, children: [
          SizedBox(height: height! / 20.0),
          SvgPicture.asset(DesignConfig.setSvgPath("favourite_empty_icon")),
          SizedBox(height: height! / 20.0),
          Text(StringsRes.noFavouriteYet,
          ...
          Text(StringsRes.noFavouriteYetSubTitle,
              textAlign: TextAlign.center,
              maxLines: 2,
                    ...
                    headerSliverBuilder: (BuildContext context, bool innerBoxIsScrolled) {
                      return <Widget>[
                        SliverAppBar(
                          ...
                          title: Text(StringsRes.favourite,
                              textAlign: TextAlign.center,
                          ...
                          bottom: PreferredSize(
                            child: Container(
                              ...
                              child: TabBar(
                                ...
                                indicator: DesignConfig.boxDecorationContainer(ColorsRes.red, 15.0),
                                tabs: [
                                  Tab(
                                    child: Text(StringsRes.products, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500)),
                                  ),
                                  Tab(
                                    child: Text(StringsRes.restaurants, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500)),
                                  ),
                                ],
                              ),
                            ),
                      ...
                      child: TabBarView(
                        children: [
                          RefreshIndicator(
                              ...
                              color: ColorsRes.red,
                              child: SingleChildScrollView(physics: const AlwaysScrollableScrollPhysics(), child: topDeal())),
                          RefreshIndicator(
                              onRefresh: refreshRestaurantsNearByList,
                              color: ColorsRes.red,
                              child: SingleChildScrollView(child: favouriteRestaurants())),
                        ],
                      ),
                    ),
                  ),
