...
class Media extends CI_Controller
{
    ...
    {
		if(!$this->ion_auth->logged_in() && !$this->ion_auth->is_partner() && ($this->ion_auth->partner_status() == 2 || $this->ion_auth->partner_status() == 7)){
            redirect('partner/login', 'refresh');
            exit();
        }
        ...
        for ($i = 0; $i < $other_image_cnt; $i++) {
            if (!empty($_FILES['documents']['name'][$i])) {
                ...
                if (!$other_img->do_upload('temp_image')) {
                    ...
                    $temp_array['partner_id'] = $this->ion_auth->get_user_id();
